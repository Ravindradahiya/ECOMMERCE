	...
	<body style="margin:0;padding:0; background:#fff; font-family: Arial, Verdana, Sans-Serif;"> 
		...
		<table width="100%">
			<tr>
				...
				<td colspan="1">Introduce On</td>
				<td colspan="1">Image</td>
				<td colspan="1">Action</td>
			</tr>
			<c:forEach items="${item}" var="i">
			<tr>
				<td colspan="1">${i.categoryName}</td>
				<td colspan="1">${i.name}</td>
				<td colspan="1">${i.description}</td>
				<td colspan="1">${i.stock}</td>
				<td colspan="1">${i.launch}</td>
				<td colspan="1">Price</td>
				<td colspan="1">${i.createdOn}</td>
				<td colspan="1">
					<img style="height: 30px; width: 30px;" src="<c:out value='${pageContext.servletContext.contextPath}'/>/<%= commonConstant.itemPageSnap %>/${i.imageA}"/>
				</td>
				<td colspan="1">
					<a href="productInfo.html?id=${i.id}"><font color="blue"><b>View</b></font></a>
				</td>
			</tr>
			</c:forEach>
		</table>
